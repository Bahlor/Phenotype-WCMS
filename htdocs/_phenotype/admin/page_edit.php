...
// Initalisierung des fuer alle Tabs gleichermassen gueltigen Formulars
    ...
    if ($block_nr==0 AND $mySUser->checkRight("elm_pageconfig"))
    {
	  ...
	  $d		=	PhenotypePeer::getRecords(1,true,'dat_bez',array(),'');
	  $data		=	array();
	  $data[]	=	'..';
	  foreach($d as $co) {
	  ...
	  // slider 
	  $html	=	$myLayout->workarea_form_select2('','slider',$myPage->get('slider'),$data);
	  $myLayout->workarea_row_draw('Slide-Galerie',$html);
...
?>
 ...
 while ($row=mysql_fetch_array($rs))
 {
    ...
    if ($block_nr>0 AND $block_nr<77)
    {
      ...
      $myLayout->workarea_start_draw();
    ?>
    <table width="100%" border="0" cellpadding="0" cellspacing="0" id="component-sort">
    <tr>
      <td colspan="4">&nbsp;</td>
    </tr>
		<?php
...
<table width="100%" border="0" cellpadding="0" cellspacing="0">
        ...
        $(document).ready(function() {
	        ...
	        $('#component-sort').sortable({
		        'axis':	'y',
		        'items':'.component-block',
		        'handle':'.drag',
		        'helper':'clone',
		        'containment':'#component-sort',
		        'opacity':0.5,
		        'start':function() {
