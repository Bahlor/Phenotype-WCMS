...
class PhenotypeBackend_Editor_Media_Standard extends PhenotypeBackend_Editor
{
	...
	function displaySearchForm()
	{
		global $myPT;
		?>
		<table width="260" class="sidebar" border="0" cellpadding="0" cellspacing="0">
        <tr>
...
?>			
	...
	function renderImport_Step1()
	{
		...
		?>
		<form action="backend.php" method="post" enctype="multipart/form-data" id="fileupload" name="editform" <?php if ($block_nr=="p"){ ?>onsubmit="return false;"<?php } ?>>
        <!--<input type="hidden" name="page" value="Editor,Media,import">
        <input type="hidden" name="grp_id" value="<?php echo $this->grp_id ?>">
        <input type="hidden" name="folder" value="<?php echo $this->folder ?>">
		<input type="hidden" name="type" value="<?php echo $this->type ?>">				   				   
		...
		<table width="100%" border="0" cellpadding="0" cellspacing="0">
		<tr>
             <td width="60" class="padding30" valign="top"><p><strong>&nbsp;</strong></p>
             </td>
             <td width="509" class="formarea">
              <?php
              $md5hash=""; 
              $target_url = ADMINFULLURL. "backend.php?page=Editor,Media,jqupload&usr_id=" . $mySUser->id ."&PHPSESSID=".session_id();
              echo $myLayout->workarea_form_jqupload(400,400,$save_path,$target_url,"Drag & Drop - Upload",$md5hash);?>
			 </td>
			 <td width="60" class="padding30" valign="top"><p><strong>&nbsp;</strong></p>
             </td>
        </tr>
			...
			if ($file != "." && $file != ".." && (!is_dir($path."/".$file)))
			{
				...
				<table width="100%" border="0" cellpadding="0" cellspacing="0">
		          <tr>
		            ...
		            <p><strong>
		            <?php
		            if (mb_substr($file,-4)==".jpg"	||	mb_substr($file,-4)==".png")
		            {
		            ?>
	...
	function jqupload() {
		...
		require_once(CLASSPATH.'upload.class.php');
		$upload_handler	=	new	UploadHandler(array('upload_dir'=>$save_path,'upload_url'=>MEDIABASEFULLURL.'import/'.$myRequest->getI('usr_id').'/'));
