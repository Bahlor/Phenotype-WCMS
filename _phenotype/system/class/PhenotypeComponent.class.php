...
class PhenotypeComponentStandard extends PhenotypeBase
{
	...
	function form_date($title, $property)
	{
	
		$this->_form[] = array(
		"form_method" =>"form_date",
		"property" =>$property,
		"title" =>$title
		);
	}
	...
	protected function _form_date_display($_fconfig)
	{
		$title = $_fconfig["title"];
		$property = $_fconfig["property"];
		...
		$formname = $this->formid . $property;
		echo $this->myLayout->workarea_form_date($title,$formname,$this->get($property));
	}
	...
	protected function _form_date_fetch($_fconfig)
	{
		$property = $_fconfig["property"];
		$this->set($property,($this->fget($property) != '') ? strtotime($this->fget($property)):'');
	}
