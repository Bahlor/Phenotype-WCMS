...
<?php
	 ...
	 */
	function tab_draw($item,$x=680,$shadow_unten=0,$shadow_rechts=1,$class='sidebar')
	{
	if($x > 300) { $class="main-content-frame";	}
	?>
	<table width="<?php echo $x ?>" class="<?=$class?>" border="0" cellpadding="0" cellspacing="0">
	<tr>
...
<table width="680" border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>
	...
	function workarea_form_date($bez,$name,$val) {
		$html = '';
		if($bez!="") { $html.= $bez.'<br />'; }
		if(!empty($val)) { if(!is_numeric($val)) { $val = strtotime($val); } }
		$html	.=	'<input type="text" name="'.$name.'" value="'.date("d.m.Y",$val).'" size="9" class="input" onchange="ajax_d'.$name.'_doit(this.value,0)">';
		$html	.=	'<input type="hidden" name="d'.$name.'" value"'.val.'" />';
		$html	.=	'<a href="javascript:flip(\'divajax_d'.$name.'\')"><img src="img/b_kalender_tr.gif" width="18" height="18" border="0" align="absmiddle"></a>';
		$html	.=	'<div id="divajax_d'.$name.'" style="position:absolute;visibility:hidden"></div>';
		$html	.=	'<script type="text/javascript">';
		$html	.=	'var ajax_d'.$name.' = new sack();';
		$html	.=	'var error=0;';
		$html	.=	'function ajax_d'.$name.'_doit(date,timestamp) {';
		$html	.=	'ajax_d'.$name.'.resetData();';
		$html	.=	'ajax_d'.$name.'.requestFile = "backend.php";';
		$html	.=	'ajax_d'.$name.'.method = "GET";';
		$html	.=	"ajax_d".$name.".element = 'divajax_d".$name."';";
		$html	.=	'ajax_d'.$name.'.setVar("page","Editor,Content,selector_date");';
		$html	.=	'ajax_d'.$name.'.setVar("d",date);';
		$html	.=	'ajax_d'.$name.'.setVar("t",timestamp);';
		$html	.=	"ajax_d".$name.".setVar('e','d".$name."');";
		$html	.=	'ajax_d'.$name.'.runAJAX();';
		$html	.=	'}';
		$html	.=	'function setDate_d'.$name.'(date){';
		$html	.=	'document.forms.editform.'.$name.'.value=date;';
		$html	.=	'hide("divajax_d'.$name.'");';
		$html	.=	'ajax_d'.$name.'_doit(date,0);';
		$html	.=	'}';
		$html	.=	'</script>';
		$html	.=	'<script type="text/javascript">';
		$html	.=	'ajax_d'.$name.'_doit("'.$val.'",1)';
		$html	.=	'</script>';
		$html.='<br />';
		return $html;
	}
...
?>
	<!-- FCKEditor -->
	<script type="text/javascript" src="<?php echo(ADMINURL); ?>lib/redactor/redactor.min.js"></script>
	<!-- /FCKEditor -->
<?php
			} else
			{
...
<?php
  ...
  ?>
	...
	<table width="680" class="main-content-frame" border="0" cellpadding="0" cellspacing="0">
      <tr>
...
?>			
	...
	function displayTreeNavi($myTree,$ext_id=-1,$x=260,$class='sidebar')
	{
		$myTree->buildtree();
		if($x > 300) { $class="main-content-frame";	}
  ?>
<table width="<?php echo $x ?>" class="<?=$class?>" border="0" cellpadding="0" cellspacing="0">
  ...
  <?php
	...
	function displayTreeNaviNoShadow($myTree,$ext_id=-1,$x=260,$class='sidebar')
	{
		$myTree->buildtree();
		if($x > 300) { $class="main-content-frame";	}
  ?>
<table width="<?php echo $x ?>" border="0" class="<?=$class?>" cellpadding="0" cellspacing="0">
      <tr>
        <td class="windowMenu">
		<?php
		for ($i=0;$i<count($myTree->_flattree);$i++)
...
?>
  <?php
	...
	/**
	 * render jQuery Multi-File Upload
	 *
	 * @param integer $width
	 ...
	 */
	 ...
	 public function workarea_form_jqupload($width,$height,$save_path='/',$target_url,$msg='jQuery MultiFile Upload',$md5hash="") {
		 global $myPT,$uploaderFiles;
		 
		 $target_url	=	$target_url.'&md5='.$md5hash;

		 if(!$uploaderFiles)	{
		 	?>
		 	<link rel="stylesheet" href="<?php echo(ADMINURL); ?>lib/uploader/css/bootstrap.css">
		 	<link rel="stylesheet" href="<?php echo(ADMINURL); ?>lib/uploader/css/jquery.fileupload-ui.css">
			<script src="<?php echo(ADMINURL); ?>lib/uploader/vendor/jquery.ui.widget.js"></script>
			...
			<script src="<?php echo(ADMINURL); ?>lib/uploader/jquery.iframe-transport.js"></script>
			<!-- The basic File Upload plugin -->
			<script src="<?php echo(ADMINURL); ?>lib/uploader/jquery.fileupload.js"></script>
			<!-- The File Upload file processing plugin -->
			<script src="<?php echo(ADMINURL); ?>lib/uploader/jquery.fileupload-fp.js"></script>
			<!-- The File Upload user interface plugin -->
			<script src="<?php echo(ADMINURL); ?>lib/uploader/jquery.fileupload-ui.js"></script>
			<!-- The localization script -->
			<script src="<?php echo(ADMINURL); ?>lib/uploader/locale.js"></script>
    ...
    <tr class="template-download fade">
        ...
        {% } else { %}
            <td class="preview">{% if (file.url) { %}
                <a href="{%=file.url%}" title="{%=file.name%}" rel="gallery" download="{%=file.name%}"><img src="{%=file.url%}" width="120"></a>
            {% } %}</td>
            <td class="name">
...
$(document).ready(function() {
	$('#fileupload').fileupload({url:'<?=$target_url?>'});
});
/*]]>*/
</script>
		 <?php
